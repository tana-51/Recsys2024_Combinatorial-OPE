...
def train_reward_model_via_two_stage(
    ...
    n_slate = bandit_data["n_slate"]
    n_unique_action = bandit_data["n_unique_action"]
    action = bandit_data["slate_id"]
    action_context = bandit_data["action_context"]
    
    
    cluster_context = np.array(list(product(np.arange(n_unique_action), repeat=len(kth_element))))
    n_cluster = cluster_context.shape[0]
    
    #cluster_mat (n_slate, 2**len(kth_element) ) あるmがどのclusterに属しているか
    cluster_contents_mat = np.zeros(n_slate*n_cluster).reshape(n_slate,-1)
    ...
    reward_main -= h_hat_mat[
        np.arange(bandit_data["context"].shape[0]), bandit_data["slate_id"]
    ]
    ...
    x = np.where(cluster_contents_mat[np.arange(n_slate),:]==1)[1].tolist()
