...
def main(cfg: DictConfig) -> None:
    ...
    n_comb_action = 2**(n_unique_action)
    ...
    for bias_noise_std in bias_noise_std_list:
        estimated_policy_value_list = []
        for _ in tqdm(range(num_runs), desc=f"bias_noise_std={bias_noise_std}..."):
            ...
            for num_kth_order in kth_order_for_opt:
                ...
                opcb_noisy_mse = squared_bias + np.random.normal(loc=0, scale = bias_noise_std) + sample_variance
        ...
        result_df["bias_noise_std"] = bias_noise_std
    ...
    result_df.to_csv("bias_noise.csv")
