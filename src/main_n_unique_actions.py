...
@hydra.main(config_path="../conf",config_name="config_n_unique_actions")
def main(cfg: DictConfig) -> None:
    ## 実験設定
    num_runs = cfg.num_runs #できるだけ大きい方が良い
    num_data = cfg.num_data 
    num_unique_actions_list = cfg.num_unique_actions_list
    n_optimize = cfg.n_optimize


    result_df_list = []
    summarize_optimized_k = []
    for num_unique_action in num_unique_actions_list:
        ...
        n_comb_action = 2**(num_unique_action)
        np.random.seed(cfg.random_state)
        unique_action_set = np.arange(num_unique_action)
        kth_order_set = np.arange(1, num_unique_action)
        ...
        kth_order_true_list = []
        for _ in tqdm(range(num_runs), desc=f"num_unique_action={num_unique_action}..."):
        ...
        result_df["num_unique_action"] = num_unique_action
    ...
    result_df.to_csv("n_unique_actions.csv")

    df_optimized_k = DataFrame(summarize_optimized_k)
    df_optimized_k["num_unique_action"] = num_unique_actions_list

    df_optimized_k.to_csv("action_optimized_k.csv")
